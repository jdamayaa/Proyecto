...
def obtener_temp_set():
    try:
        with open('sets.json', 'r') as file:
            data = json.load(file)  # Cargar los datos desde el archivo JSON
            temp_set = data[0] if len(data) > 0 else None    # Obtener el valor de "temp" (si no existe, devuelve None)
            return temp_set
    except OSError:
        print("El archivo sets.json no existe o no se pudo leer.")
        return None
...
def obtener_hum_set():
    try:
        with open('sets.json', 'r') as file:
            data = json.load(file)  # Cargar los datos desde el archivo JSON
            hum_set = data[1] if len(data) > 0 else None    # Obtener el valor de "temp" (si no existe, devuelve None)
            return hum_set
    except OSError:
        print("El archivo sets.json no existe o no se pudo leer.")
        return None
    
temp_set = obtener_temp_set()
hum_set = obtener_hum_set()
...
def cargar_datos_set():
    global sets
    try:
        with open ('sets.json', 'r') as f:
            sets = json.load(f)
            print("Datos cargados sets: ", sets)
    except OSError:
        sets = []

def guardar_datos_set():
    print('Guardando datos en el JSON set')
    global sets
    #while True:
    try:
        with open('sets.json', 'w') as f:    #abre o crea el archivo.json la f sirve para interactuar con el archivo, la w significa q esta en modo escritura
            json.dump(sets, f)             #Escribe la lista valores en el archivo en formato JSON
            print("datos guardados exitosamente en JSON sets-------------------------------------------------------")
    except Exception as e:
