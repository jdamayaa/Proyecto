...
async def conectar_red():
    ...
    while not wlan or wlan.status() != 3:
        ...
        while True:
            ...
            led.on()
    try:
        # Sincronizar la hora desde un servidor NTP
        ntptime.settime()
        sleep(2)
        print("Hora sincronizada con NTP:", time.localtime())
        # Ajustar la hora a la zona horaria
        hora_utc = time.localtime()  # Hora UTC sincronizada
        hora_local = ajustar_hora_zona(hora_utc)  # Ajuste según la zona horaria
    ...
    linea1 = 'Conexion anvil'
    linea2 = 'Exitosa'
    pantalla(linea1,linea2)
    await a.sleep(2)  # Espera 5 segundos para evitar múltiples conexiones


    
def ajustar_hora_zona(hora_utc):
    global hora
    global hora_ajustada
    # Ajustar las horas según la diferencia horaria (por ejemplo, UTC-5)
    DIFERENCIA_HORARIA = -5  # Cambia este valor según tu zona horaria
    hora_ajustada = list(hora_utc)
    hora_ajustada[3] += DIFERENCIA_HORARIA  # Ajusta la hora (índice 3 es la hora)
    
    # Asegurarse de que el día no se desajuste
    if hora_ajustada[3] < 0:
        hora_ajustada[3] += 24
        hora_ajustada[2] -= 1  # Restar un día si la hora se ajusta antes de la medianoche
    elif hora_ajustada[3] >= 24:
        hora_ajustada[3] -= 24
        hora_ajustada[2] += 1  # Avanzar un día si la hora se ajusta después de la medianoch
    hora = True
...
async def pico_fn1():
    ...
    try:
        ...
        # Verificar que 'datos' sea una lista
        if isinstance(datos, list):
            # Obtener el tiempo actual en segundos
            ahora = time.time()

            # Calcular el tiempo de hace 2 días (48 horas)
            dos_dias_atras = ahora - (2 * 24 * 3600)

            # Filtrar los datos para que estén dentro de los últimos 2 días
            datos_filtrados = []
            for registro in datos:
                # Extraer año, mes, día, hora y minuto
                año, mes, dia, hora, minuto = registro[0]

                # Crear un objeto de fecha para la comparación con mktime
                fecha_registro = time.mktime((año, mes, dia, hora, minuto, 0, 0, 0, 0))

                # Verificar si la fecha está dentro del rango de los últimos 2 días
                if fecha_registro >= dos_dias_atras:
                    datos_filtrados.append(registro)

            # Obtener los últimos 20 elementos (o menos si no hay suficientes)
            ultimos_20 = datos_filtrados[-20:] if len(datos_filtrados) >= 20 else datos_filtrados
        ...
        # Manejar errores de archivos (ej. archivo no encontrado o problemas de lectura)
...
async def recolectar_valores():
    ...
    while True:
        await a.sleep(30)
        #print("entrando a recolectar valores en el while")
        if hora ==  True:
            ...
            tim = time.localtime()
            fecha_hora = ajustar_hora_zona(tim)
            #fecha_hora = fecha_horat[0]
