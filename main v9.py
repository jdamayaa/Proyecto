...
def menu_ppal():
    ...
    while True:
        #display.ClearScreenCursorHome()  
        display.WriteLine('Temp°C:'+str(temp)+' Set:' + str(temp_set) ,1)
        display.WriteLine('Hume %:'+str(hum)+' Set:' + str(hum_set) ,2)
...
def menu1_1():
    ...
    while True:
        ...
        display.WriteLine('>Temp Set: '+str(temp)+'°C',1)
        display.WriteLine(' Hum  Set: '+str(hum)+'% ',2)
        #display.CursorMayor(1)
        lectura_teclado()
        #print(opcion_teclado)
        if opcion_teclado == 'seleccionar':
